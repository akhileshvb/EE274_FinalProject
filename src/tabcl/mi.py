...
def compute_hashed_mi(
	...
	"""Approximate MI using bucketing. Uses quantiles for numeric data, hashing otherwise."""
	if x.shape[0] != y.shape[0]:
		raise ValueError("x and y must have same length")
	n_total = x.shape[0]
	...
	# Quantiles preserve correlation better than hashing for numeric data
	numeric_num_buckets = min(num_buckets, 256)
...
def estimate_edge_weight(n_rows: int, x: np.ndarray, y: np.ndarray, mdl_bits: float) -> float:
	"""Edge weight = n * I(X;Y) - MDL cost (both in bits)."""
